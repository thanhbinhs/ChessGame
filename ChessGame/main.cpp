...
const float SCREEN_MARGIN = 40.0f;
...
void GameManager::toCapture(int n) {
...
{ 
 ...
 posS.push(newPos);
 nS.push(n);
 int y=int((newPos-offset).y/size_);//kiem tra xem co phong hau hay khong
 //phong hau cho tot
 if(y==0 && f[n].index==6){
  ...
  toCapture(n);
  f[n].cost=90;
  f[n].s.setTextureRect(IntRect( 3*size_,size_,size_,size_));
 }
 if(y==7 && f[n].index==-6){
  nS.push(-100);
  Board b(window);
  ...
  toCapture(n);
  f[n].cost=-90;
  f[n].s.setTextureRect(IntRect( 3*size_,0,size_,size_));
 }
